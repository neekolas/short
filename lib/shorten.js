...
shorten.generate = function(document) {
  var generatePromise
    , promise = new Promise()
...
shorten.chooseDestination = function(shortObj) {
    var originalURL = shortObj.URL;
    if (!shortObj.bizrateBid) {
...
shorten.generateRoute = function(req, res, skimlink, addWantProduct, log) {
  ...
  if (!url.parse(requestURL).hostname) {
      log.log({event: 'Shorterror', text: 'Invalid URL'});
      res.status(404);
      res.send('Invalid URL');
      routePromise.reject(new Error('No such short URL'), true);
  }
