...
shorten.chooseDestination = function(shortObj, req) {
    ...
    if (shortObj.skimlink) {
        destinationURL = 'http://buy.wantering.com/?id=' + shortObj.pubID + '&xs=1&url='+encodeURIComponent(originalURL)
    } else {
    ...
    addProductPromise.then(function(result) {
      ...
      if (result.bizrate) {
        ...
        besoPromise.then(function(returnObj) {
          ...
          addWantProduct.add(returnObj.destHost, returnObj.destURL, mongodbDoc.hash, true)
        });
      }
    }, function(error) {
      if (error) {
